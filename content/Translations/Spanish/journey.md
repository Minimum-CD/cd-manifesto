...
Cada proceso que agregamos debe añadir valor. Hay un valor obvio en realizar pruebas de rendimiento, a menos que estemos exigiendo un rendimiento a un nivel que no es necesario para el caso de uso. Sin embargo, antes de definir la documentación del CAB como requisito para ser "desplegable", ¿por qué existe ese proceso? Una razón común para la realización de esa condición es una regla de cumplimiento de "dos pares de ojos en cada cambio". Sin embargo, eso puede ser validado por automatización sin la necesidad de agrupar cambios y esperar un acuerdo.
...
Los equipos no son buenos en pruebas inicialmente por la misma razón que no son buenos en cualquier otra habilidad el día 1. Lleva tiempo. Comienza ahora.

### Las tareas individuales son demasiado grandes

CI significa que estamos integrando características parcialmente completadas continuamente. TDD nos ayuda a aprender cómo descomponer tareas en cambios muy pequeños y publicables que no rompen los comportamientos existentes. Sin embargo, el hábito inicial a menudo es esperar para integrar cambios hasta que estén "completos". Esto resulta en conjuntos de cambios grandes que son más difíciles de revisar. Esto hace que la revisión de código tome más tiempo, requiera más reelaboración y reduzca la capacidad del revisor para detectar problemas. Los [métodos de codificación evolutivos](/minimumcd/ci/#recommended-practices) permiten la liberación de características incompletas hasta que estén listas y nos permiten reducir el tamaño de los cambios. Al centrarnos como equipo en descomponer las tareas en horas de trabajo en lugar de días y usar técnicas de ingeniería para controlar la liberación, hemos mejorado la claridad, reducido los conjuntos de cambios y aumentado la calidad.

### Las historias son demasiado grandes y carecen de criterios de aceptación comprobables

El verdadero desarrollo guiado por pruebas comienza con la conversación del equipo sobre la historia. Es común que a los equipos se les entreguen historias y se espere que las "estimen" y las codifiquen. Eso impacta negativamente la calidad porque el equipo carece de la información necesaria para entregar un buen resultado. En su lugar, una historia sin refinar debería ser el comienzo de la conversación sobre los objetivos del producto, la implementación y la validación.

La [descomposición del trabajo](https://dojoconsortium.org/docs/work-decomposition/work-breakdown/) y las pruebas son habilidades fundamentales para CI. Esto se hace mejor centrándose en el [Desarrollo Guiado por Comportamiento](https://dojoconsortium.org/docs/work-decomposition/behavior-driven-development/) en lugar del "formato de historia". Necesitamos criterios de aceptación que impulsen las pruebas para que tengamos declaraciones declarativas sobre exactamente cómo debería comportarse el cambio. Más allá de proporcionar la claridad requerida, también reduce la carga cognitiva de escribir pruebas, mejora el trabajo en equipo y nos da segmentos naturales para "dividir" para que podamos reducir las historias a 1-2 días de esfuerzo. Esto también nos protege de cambios de prioridades que fuerzan a "aparcar" trabajo incompleto; otro importante contribuyente a la mala calidad.

### El equipo utiliza un sistema push para el trabajo

Un equipo nunca asigna trabajo. Un equipo trabaja junto para entregar la mayor prioridad en el backlog. Si hay un proceso donde el trabajo se asigna al comienzo de una iteración, entonces cada persona solo se enfoca en su trabajo asignado en lugar de en los objetivos del equipo. Esto impacta negativamente todo en nuestro flujo de trabajo. Todos en el equipo deberían estar invertidos en todo lo que hace el equipo. Eso hace que la descomposición del trabajo, la revisión del código, las pruebas y todo lo demás sea mucho más efectivo.

## Reemplazar lo Manual Con automatización

Utilizando nuestras definiciones de "desplegable" y "publicable", necesitamos comenzar a reemplazar la verificación manual con automatizada. Esto lleva tiempo. También necesitamos cambiar los flujos de trabajo. Por ejemplo, algunas cosas son manuales hoy que necesitarán ser manuales para siempre. Ninguna de esas cosas debería estar verificando reglas o listas de verificación. Podemos automatizar esas.

Las Pruebas de Aceptación del Usuario es una actividad común previa a la liberación. Sin embargo, generalmente podemos reemplazar eso con pruebas continuas de usabilidad, pruebas exploratorias y demostraciones frecuentes. Todas estas deberían estar ocurriendo paralelamente a la entrega, no como compuertas de etapa. En algunos casos, hay razones regulatorias por las que UAT existe como una compuerta de etapa. Sin embargo, debemos tener en cuenta que cualquier cosa que agregue resistencia aumentará el tamaño del lote, reducirá la retroalimentación y aumentará las tasas de creación de defectos. ¿Cómo podemos optimizar ese proceso de UAT para hacerlo más pequeño y más frecuente para reducir este riesgo?

Mejora implacablemente y reemplaza procesos con automatización. Sigue reduciendo el tamaño de las liberaciones para descubrir más desafíos a resolver. Este proceso crea un efecto bola de nieve donde estamos descubriendo y mitigando problemas en nuestros procesos que impactan la calidad mientras automatizamos pasos manuales que quitan tiempo al desarrollo. Esto a su vez nos da más tiempo para centrarnos en los objetivos del producto y en mejoras adicionales del flujo de entrega. Los resultados de esto son mejoras dramáticas en la cultura del equipo, la excelencia en ingeniería y los objetivos de negocio.
...
Esto no se puede enfatizar lo suficiente. El propósito del pipeline es validar completamente que un artefacto es digno de producción o rechazarlo. No corras hacia la entrega diaria sin primero construir confianza en tu capacidad para detectar fallos. Mueve la validación rápida al escritorio y ejecútala nuevamente cuando intentes fusionar código al trunk y nuevamente cuando se realicen cambios en el trunk.

Las pruebas no se limitan a pruebas funcionales. Necesitamos probar la seguridad, el cumplimiento y todo lo demás que se necesita para validar el artefacto en nuestro contexto.

Establece presupuestos de error y no los excedas. Si estamos impulsando características y nuestro presupuesto de error está roto, necesitamos centrarnos en fortalecer nuestros pipelines. Cuando las cosas se rompen en producción, fortalecemos nuestros pipelines. Cuando encontramos un caso límite en pruebas exploratorias, fortalecemos el pipeline. Nuestro objetivo principal es construir puertas de calidad eficientes y efectivas. Solo entonces podremos movernos rápidamente.
