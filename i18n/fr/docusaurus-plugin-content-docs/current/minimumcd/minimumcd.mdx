...
Nous, [les signataires](..//docs/minimumcd#signatories), estimons qu'une définition minimale de la Livraison Continue (Continuous Delivery, CD) est requise afin d'améliorer les flux de livraisons. Bien que chaque contexte soit unique, il y a des pratiques universelles. En les définissant, nous pouvons&nbsp;:
...
- Le pipeline décide de la diffusion des modifications, son verdict est [définitif](/docs/faq/#why-should-the-pipeline-be-definitive-for-deploy)
...
- [L'artefact ne change jamais](..//docs/minimumcd/immutable/). Il n'y a aucun changement humain après le commit.
...
- [La configuration de l'application](/docs/faq/#what-is-application-configuration) est déployée avec l'artefact
...
- [Le développement basé sur un tronc commun](..//docs/minimumcd/tbd/)
...
[Le développement basé sur un tronc commun](..//docs/minimumcd/tbd/) est le modèle de branche requis pour répondre à la définition d'Intégration Continue.
...
Pour plus d'informations sur la Livraison Continue Minimale et des réponses à d'autres questions courantes, [veuillez lire la FAQ](/docs/faq/).

## Vous souhaitez contribuer ou devenir signataire ?

Les signataires ont signé le [document original en anglais](..//docs/minimumcd#signatories), et la liste des noms sera tenue à jour uniquement sur celui-ci.
