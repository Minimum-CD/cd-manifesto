...
- [Immutable artifact](/docs/minimumcd/immutable/) (no human changes after commit)
...
[Trunk-based development](/docs/minimumcd/tbd) is the branching pattern required to meet the definition
...
export const ListPeople = ({data}) => {
  return(<>
  {
    data.map(({name, contact}, index) => (<><a style={{margin: "0 5px 0 0"}} href={contact}>{name}</a> {index !== data.length-1 && "•"} </>))
  } 
  </>)
}
...
<ListPeople data={require("/data/names/contributors.json")} />

## Signatories

<ListPeople data={require("/data/names/signatures.json")} />
